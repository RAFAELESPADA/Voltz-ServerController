...
public class TellCommand extends Command {
  ...
  public void execute(CommandSender sender, String[] args) {
    ...
    if (targetPlayerName.equalsIgnoreCase(player.getName())) {
      sender.sendMessage(TextComponent.fromLegacyText("§cVoc§ n§o pode mandar mensagem para voc§ mesmo."));
      return;
    } 
    ProxiedPlayer targetPlayer = ProxyServer.getInstance().getPlayer(targetPlayerName);
    if (targetPlayer == null) {
      sender.sendMessage(TextComponent.fromLegacyText("§cN§o achei esse jogador."));
      return;
    } 
